...
| 文件 | 位置 | 说明 |
|------|------|------|
| `src/server.rs` | trait Server 定义 | 删除 trait 上的注解 |
| `src/server.rs` | tests 中 MockServer impl | 删除 impl 上的注解 |
| `src/axum_server.rs` | AxumServer impl | 删除 impl 上的注解 |
| `src/axum_server.rs` | AxumTlsServer impl | 删除 impl 上的注解 |
| `src/blocking_server.rs` | BlockingServer impl | 删除 impl 上的注解 |
...
| `GinConfig` | host="0.0.0.0", port=8000 | `gin_config_merge_default` |
| `GinSwaggerConfig` | schemes=["https","http"] | `gin_swagger_config_merge_default` |
...
**端口类型**:
- `GinConfig.port: i32` → `port: u16`
- `src/axum_server.rs`: 删除 `gin_config.port as u16`，直接用 `gin_config.port`
